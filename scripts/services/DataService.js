...
const getSingleCoinUrl = (id) =>
  `https://api.coinpaprika.com/v1/coins/${id}/ohlcv/today/`;

const DataService = {
  getCurrencies(callback, filter = "", sort = "rank") {
    DataService._sendRequest(COINS_URL).then((response) => {
      ...
      Promise.all(urls.map((url) => DataService._sendRequest(url, true)))
        .then((response) => {
          response.forEach((element, index) => {
            ...
            if (element !== undefined) data[index].price = element.close;
